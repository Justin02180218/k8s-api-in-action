...
import (
	...
	w "com.justin.k8s.api/deploy/k8sapi/watcher"
)
...
func main() {
	 ...
	 * 监控 Resource
	 */
	go w.PodWatcher(clientset, NAMESPACE)
	...
	// r.DeleteDeployment(clientset, articleDeployment.Namespace, articleDeployment.Name)
	...
	// r.DeleteService(clientset, articleService.Namespace, articleService.Name)

	/**
	 * 创建 Ingress
	 */
	ingressResource := &r.IngressResource{
		Namespace:   NAMESPACE,
		Name:        "blog-ingress",
		Host:        []string{"user.blog.com", "article.blog.com"},
		ServiceName: []string{userService.Name, articleService.Name},
		ServicePort: []int32{8888, 8899},
	}

	r.CreateIngress(clientset, ingressResource)
	// r.DeleteIngress(clientset, ingressResource.Namespace, ingressResource.Name)
